...
=head1 SYNOPSIS
 ...
 my @types = $mt->httpAccept('text/html, application/json;q=9')
...
The C<$accept> string is processed via L<httpAccept()|MIME::Types/"HTTP support"> to order the
...
example: 
   ...
   my @have   = map $mt->mimeTypeOf($_), bsd_glob "$fnbase.*;
   my $accept = $req->header('Accept');

=item $obj-E<gt>B<httpAcceptSelect>(($accept|\@types), (@filenames|\@filenames))

Like L<httpAcceptBest()|MIME::Types/"HTTP support">, but now we do not return a pair with mime-type
...
This module is part of MIME-Types distribution version 2.07,
